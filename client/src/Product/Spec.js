...
import { singleProduct } from '../utils/API.routes';
const Spec = () => {
    let { id } = useParams();
    //   console.log('id',id)
    ...
    useEffect(() => {
        ...
        axios.get(`${singleProduct}${id}`).then((response) => {
            // console.log("ye nya data hai",response.data.data)
    ...
    const CheckServiceability = async () => {
        axios.get('http://localhost:5000/checkPincode').then((response) => {
    ...
    // console.log("product to be mapped>>>", product);
    const onChangePin = (e) => {
                    ...
                    {product.map((item, key) => {
                        return (
                            ...
                            <div key={key} className="lg:w-4/5 mx-auto flex flex-wrap">
                                <img alt="ecommerce" className="lg:w-1/2 w-full lg:h-auto px-20  object-cover object-top rounded" src={item.product_details.image[0]} />
                                <div className="lg:w-1/2 w-full lg:pl-10 lg:py-6 mt-6 lg:mt-0">
                                    <h2 className="text-sm title-font text-gray-500 tracking-widest">{item.product_category}</h2>
                                    <h1 className="text-gray-900 text-3xl title-font font-medium mb-1">{item.brand.toUpperCase()}</h1>
                                    ...
                                    <div className="flex mt-6 items-center pb-5 border-b-2 border-gray-100 mb-5">
                                        <div className="flex">
                                            ...
                                            {item.product_details.color.map((color, key) => {
                                                return (
                                                    <>
                                                        {/* <button className="border-2 border-gray-300 rounded-full w-6 h-6 focus:outline-none"></button> */}
                                                        {color.includes('blue') && <button className={`border-2  ml-1 bg-blue-500 rounded-full w-6 h-6 focus:outline-none  ${color === 'blue' ? 'border-black' : 'border-gray-300'}`}>
                                                        </button>}
                                                        {color.includes('red') && <button className={`border-2  ml-1 bg-red-500 rounded-full w-6 h-6 focus:outline-none  ${color === 'red' ? 'border-black' : 'border-gray-300'}`}>
                                                        </button>}
                                                        {color.includes('grey') && <button className={`border-2  ml-1 bg-gray-500 rounded-full w-6 h-6 focus:outline-none  ${color === 'grey' ? 'border-black' : 'border-gray-300'}`}>
                                                        </button>}
                                                        {color.includes('pink') && <button className={`border-2  ml-1 bg-pink-500 rounded-full w-6 h-6 focus:outline-none  ${color === 'pink' ? 'border-black' : 'border-gray-300'}`}>
                                                        </button>}
                                        ...
                                        <div className="flex ml-6 items-center">
                                            ...
                                            <div className="relative">
                                                <select className="rounded border appearance-none border-gray-300 py-2 focus:outline-none focus:ring-2 focus:ring-red-200 focus:border-red-500 text-base pl-3 pr-10">
                                                    {item.product_details.size.map((size, key) => {
                                                        return (
                                                            <>
                                                                {size.includes('XS') && <option value={'XS'}>XS</option>}
                                                                {size.includes('S') && <option value={'S'}>S</option>}
                                                                {size.includes('M') && <option value={'M'}>M</option>}
                                                                {size.includes('L') && <option value={'L'}>L</option>}
                                                                {size.includes('XL') && <option value={'XL'}>XL</option>}
                                                                {size.includes('XXL') && <option value={'XXL'}>XXL</option>}
                                                            </>
                                                        )
                                                    })}
                                    ...
                                    <div className="flex">
                                        <span className="title-font font-medium text-2xl text-gray-900">₹ {item.product_details.price[0]}</span>
