        ...
        <div className='flex flex-col md:flex-row md:justify-start justify-between items-center py-1 shadow-md sticky top-0 bg-white z-10'>
            ...
            <div className="nav">
                <ul className="flex items-center space-x-4 font-medium md:text-md">
                    ...
                    {/* <Link passhref="true" key={'tshirt'} to={`/Tshirts/${'tshirt'}`}><li className='hover:text-red-500'>Tshirts</li></Link> */}
                    <Link to={`/Jeans?category=${'jeans'}`}><li className='hover:text-red-500'>Jeans</li></Link>
                    <Link to={`/Kids?category=${'kids'}`}><li className='hover:text-red-500'>Kids</li></Link>
                    <Link to={`/Home_Living?category=${'homeLiving'}`}><li className='hover:text-red-500'>Home & Living</li></Link>
                    <Link to={`/Beauty?category=${'beauty'}`}><li className='hover:text-red-500'>Beauty</li></Link>
                    <Link to={'/Add'}><li className='hover:text-red-500'>Add</li></Link>
                </ul>
            </div>
            <div className=" cursor-pointer cart absolute right-0 top-4 mx-5 flex ">

                {isAuthenticated && <Link to={'/Profile'}><MdAccountCircle className='text-xl md:text-2xl mx-2 ' onMouseOver={UserInfo}/></Link>}
                {!isAuthenticated && <Link to={'/Login'}><MdAccountCircle className='text-xl md:text-2xl mx-2' /></Link>}
